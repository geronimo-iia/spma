...
**Span contiguity / gap constraint**: when advancing to the next symbol of an Old pattern, the default is exactly `prev_new + 1`. If the pattern has a `GapConstraint` at that position, the beam allows `skip` New positions in `[min, max]` instead. Skipped positions are uncovered (contribute to E). The first symbol of a pattern can start at any New position.
...
Symbols present in the training corpus but never absorbed into any grammar pattern would have `bit_cost = 0.0`, making them uncovered-but-free at inference (E=0 false negatives). Fix: Shannon costs from the training corpus are snapshotted at the end of training. At inference, `costs[id]` falls back to the corpus snapshot when the grammar has no cost for that symbol.
...
3. Convergence: loop terminates when grammar stopped growing and no new patterns were added this pass. No T-epsilon check (unreliable — cost recomputation between measurements produces noise that never converges).
...
Each column = one position in New. Each row = one Old pattern used. Unmatched New symbols show with no Old pattern beneath. `E > 0` is the anomaly signal.

## Train/infer workflow

```bash
spma train normal_logs.txt      # trains grammar, saves model JSON
spma infer anomaly.txt          # loads grammar, aligns, scores — no grammar update
```

Grammar persistence: JSON serialization via serde. On load, the symbol index is rebuilt from serialized patterns.

## Hierarchical grammar (N-level)

The engine runs N levels: level-0 induces atom patterns; level-k induces patterns over level-(k-1) pattern IDs. `SymbolRef::Pattern(pid)` references a lower-level pattern. `InferResult::level_e_norms` gives per-level normalized E. See [docs/grammar-spec.md](grammar-spec.md) for the data model.
